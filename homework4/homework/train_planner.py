...
#                 image, waypoints, mask = (
#                     batch["image"].to(device),
#                     batch["waypoints"].to(device),
#                     batch["waypoints_mask"].to(device),
#                 )
#                 predictions = model(image)  # Pass only the image
#             else:
...
#                     batch["waypoints_mask"].to(device),
#                 )
#                 predictions = model(track_left, track_right)  # Pass lane boundaries
...
#             # loss = weighted_loss(predictions, waypoints, mask, alpha=0.95)  # Apply mask
#             loss.backward()
...
#                 loss = criterion(predictions[mask], waypoints[mask])
#                 # loss = weighted_loss(predictions, waypoints, mask, alpha=0.8)  # Apply mask
#                 val_loss += loss.item()
...
#         if lateral_error < 0.6 and longitudinal_error < 0.2:
...
def train_model(
    ...
    writer.close()
    print(f"Training complete. Best lateral error: {best_lateral_error:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, required=True, choices=["mlp_planner", "cnn_planner", "transformer_planner"], help="Model to train")
